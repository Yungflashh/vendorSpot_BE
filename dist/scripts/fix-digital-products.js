...
async function fixDigitalProducts() {
    try {
        logger_1.logger.info('🔧 Starting digital products fix...');
        // Connect to database
        ...
        for (const product of digitalProducts) {
            ...
            if (!digitalFileInfo) {
                logger_1.logger.warn(`⚠️  No digitalFile data for: ${product.name}`);
                skipped++;
                continue;
            }
        ...
        if (updated > 0) {
            ...
            logger_1.logger.info('   - Download URL: ✅');
            logger_1.logger.info('   - File Name: ✅');
            logger_1.logger.info('   - File Size: ✅');
            logger_1.logger.info('   - File Type: ✅');
            logger_1.logger.info('   - Version: ✅');
        }
        process.exit(0);
    }
    catch (error) {
        logger_1.logger.error('❌ Error fixing digital products:', error);
        process.exit(1);
    }
}
// Run the fix
fixDigitalProducts();
//# sourceMappingURL=fix-digital-products.js.map