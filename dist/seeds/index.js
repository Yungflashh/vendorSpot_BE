...
async function seedDatabase() {
    try {
        logger_1.logger.info('🌱 Starting comprehensive database seeding...');
        // Connect to database
        await (0, database_1.connectDB)();
        // Clear existing data
        logger_1.logger.info('🗑️  Clearing existing data...');
        await Promise.all([
            User_1.default.deleteMany({}),
            Product_1.default.deleteMany({}),
            Category_1.default.deleteMany({}),
            VendorProfile_1.default.deleteMany({}),
            Additional_1.Coupon.deleteMany({}),
        ]);
        // Seed Categories
        ...
        const createdUsers = await User_1.default.insertMany(hashedUsers);
        ...
        logger_1.logger.info('🏪 Seeding vendor profiles with ratings...');
        ...
        const createdVendorProfiles = await VendorProfile_1.default.insertMany(vendorProfilesWithUsers);
        logger_1.logger.info(`✅ Created ${createdVendorProfiles.length} vendor profiles with ratings`);
        // Combine all products
        const allProducts = [...physicalProducts, ...digitalProducts];
        // Seed Products with ratings, features and specifications
        logger_1.logger.info('📦 Seeding 40 products (20 physical + 20 digital) with keyFeatures and specifications...');
        ...
        const createdProducts = await Product_1.default.insertMany(productsWithRefs);
        logger_1.logger.info(`✅ Created ${createdProducts.length} products with features and specifications`);
        logger_1.logger.info(`   - Physical products: ${physicalProducts.length}`);
        logger_1.logger.info(`   - Digital products: ${digitalProducts.length}`);
        // 🔥 UPDATE PRODUCT COUNTS FOR EACH CATEGORY 🔥
        logger_1.logger.info('📊 Updating category product counts...');
        // Group products by category and count them
        const categoryProductCounts = createdProducts.reduce((acc, product) => {
        ...
        for (const [categoryId, count] of Object.entries(categoryProductCounts)) {
            await Category_1.default.findByIdAndUpdate(categoryId, { productCount: count });
            const category = await Category_1.default.findById(categoryId);
            logger_1.logger.info(`   ✅ ${category?.name}: ${count} products`);
        }
        logger_1.logger.info('✅ Category product counts updated');
        // Seed Coupons
        logger_1.logger.info('🎟️  Seeding coupons...');
        const createdCoupons = await Additional_1.Coupon.insertMany(coupons);
        logger_1.logger.info(`✅ Created ${createdCoupons.length} coupons`);
        // Summary
        logger_1.logger.info('\n✨ Database seeding completed successfully!\n');
        logger_1.logger.info('📊 Summary:');
        logger_1.logger.info(`   - Categories: ${createdCategories.length}`);
        logger_1.logger.info(`   - Users: ${createdUsers.length}`);
        logger_1.logger.info(`   - Vendor Profiles: ${createdVendorProfiles.length} (with ratings)`);
        logger_1.logger.info(`   - Products: ${createdProducts.length} (with ratings, features & specs)`);
        logger_1.logger.info(`   - Coupons: ${createdCoupons.length}`);
        logger_1.logger.info('\n👤 Test Accounts:');
        logger_1.logger.info('   Admin:    admin@vendorspot.com / Admin123!');
        logger_1.logger.info('   Vendor 1: vendor@vendorspot.com / Vendor123!');
        logger_1.logger.info('   Vendor 2: vendor2@vendorspot.com / Vendor123!');
        logger_1.logger.info('   Vendor 3: vendor3@vendorspot.com / Vendor123!');
        logger_1.logger.info('   Vendor 4: vendor4@vendorspot.com / Vendor123!');
        logger_1.logger.info('   Customer: customer@vendorspot.com / Customer123!');
        logger_1.logger.info('\n🏪 Vendor Profiles with Ratings:');
        logger_1.logger.info("   Jane's Electronics Hub     - 4.9⭐ (500 reviews, 1,250 sales)");
        logger_1.logger.info("   Fashion Forward by Sarah   - 4.8⭐ (380 reviews, 890 sales)");
        logger_1.logger.info("   David's Home Store         - 4.7⭐ (250 reviews, 420 sales)");
        logger_1.logger.info("   Emma's Digital Academy     - 4.9⭐ (620 reviews, 1,850 sales)");
        logger_1.logger.info('\n🎟️  Test Coupons:');
        logger_1.logger.info('   WELCOME20 - 20% off (min ₦50k)');
        logger_1.logger.info('   SAVE5K    - ₦5,000 off (min ₦100k)');
        logger_1.logger.info('   FLASH50   - 50% off (min ₦20k)');
        logger_1.logger.info('\n📦 Products Created:');
        logger_1.logger.info('   - 20 Physical products (with stock, ratings, features & specs)');
        logger_1.logger.info('   - 20 Digital products (courses, templates, e-books, features & specs)');
        logger_1.logger.info('   - All products have ratings (4.3-4.9 stars)');
        logger_1.logger.info('   - All products have reviews (120-620 reviews)');
        logger_1.logger.info('   - All products have keyFeatures (3-5 bullet points)');
        logger_1.logger.info('   - All products have specifications (8-10 key-value pairs)');
        logger_1.logger.info('\n📊 Category Product Counts:');
        for (const category of createdCategories) {
            const updated = await Category_1.default.findById(category._id);
            logger_1.logger.info(`   - ${updated?.name}: ${updated?.productCount} products`);
        }
        logger_1.logger.info('\n🖼️  All images use picsum.photos for compatibility');
        logger_1.logger.info('✅ Ready to fetch top vendors by rating, sales, or products!');
        logger_1.logger.info('✅ Ready to display full product details with features and specifications!');
        logger_1.logger.info('✅ Category product counts are now accurate!');
        process.exit(0);
    }
    catch (error) {
